		...
		public override function updateSprite():void {
			...
			if( true) {
					...
					if (this.sprite_type == AGMode.S_GATOR) {
						...
						if (this.active ==true  ) {
							xx = Math.floor(int ( this.x / 16));
							yy = Math.floor(int ( this.y / 16)) ;
							...
							if(this.facingRight == true ) {
								...
								if (xx + 3 < myMode.myHoriz && yy + 2 < myMode.myVert || true ) {
									...
									if(myMode.myInvisible[yy][xx+2] + myMode.mapcheat == AGMode.B_BLOCK  ) markerTest = true;//TRUE;
									if(myMode.myInvisible[yy][xx+2] + myMode.mapcheat == AGMode.B_MARKER ) markerTest = true;// TRUE;
									if(myMode.myInvisible[yy+1][xx+2]  == 0) markerTest = true;//TRUE;
								}
								// turn monster
								if (this.x > myMode.myHoriz * 16  - 32 || markerTest == true ) {
								...
								if (xx -1 > 0 && yy + 2 < myMode.myVert || true) {
									if(myMode.myInvisible[yy][xx] + myMode.mapcheat == AGMode.B_BLOCK) markerTest = true;//TRUE;
									if(myMode.myInvisible[yy][xx] + myMode.mapcheat == AGMode.B_MARKER) markerTest =true;// TRUE;
							...
							visibility = show;
							//hide monster if...
							if(this.x > myMode.scrollBGX + 64 * 16 + 32 ) {
								visibility = hide;
							}
							if (this.x < myMode.scrollBGX - 32) {
								visibility = hide;
							}
							if (this.y > myMode.scrollBGY + 48 * 16 + 32) {
								visibility = hide;
							}
